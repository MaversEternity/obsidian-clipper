...
(function() {
	...
	// PDF in browser has closed shadow root — redirect to book-viewer for lookups
	const isPdfUrl = window.location.pathname.toLowerCase().endsWith('.pdf');
	const hasPdfEmbed = document.querySelector('embed[type*="pdf"]');
	if ((isPdfUrl || hasPdfEmbed) && window.location.href && !window.location.href.startsWith(browser.runtime.getURL(''))) {
		browser.runtime.sendMessage({
			action: 'openBookViewerForUrl',
	...
	// Collect mark.js targets: document.body + any shadow roots that contain text
	function getMarkTargets(): (HTMLElement | DocumentFragment)[] {
		const targets: (HTMLElement | DocumentFragment)[] = [document.body];
		// Walk all elements looking for shadow roots with text content
